...
***********************************************************************/
#define SUCCESS 0
#define DEVICE_NAME "ioctl_global"
#define BUF_LEN 80
...
*
*   Description : This function is called whenever an ioctl call is made by an user application.
*
*   Parameters  : file pointer,buffer pointer,count of data to be read,seek position.
*
*   Return value: number of bytes written.
...
long device_ioctl(struct file *file,unsigned int ioctl_num,unsigned long ioctl_param)
{
	...
	switch(ioctl_num)
	{
		case TEST_IOCTL_SET_VARIABLE:
			...
			my_global=(int)ioctl_param;
			printk(KERN_ALERT "\nThe value of global variable is:%d.\n",my_global);
			break;
